...
DROP TABLE IF EXISTS badminton_match_summary;
DROP TABLE IF EXISTS badminton_sets;
DROP TABLE IF EXISTS badminton_matches;

/*
  Table: badminton_matches
  Purpose: Store one row per match with team/player metadata and status
*/
CREATE TABLE IF NOT EXISTS badminton_matches (
  id INT AUTO_INCREMENT PRIMARY KEY,
  match_type ENUM('Singles','Doubles','Mixed Doubles') NOT NULL,
...
/*
  Table: badminton_sets
  Purpose: Store each saved set tied to a match (scores, timeouts, serving, winner)
*/
CREATE TABLE IF NOT EXISTS badminton_sets (
  ...
  FOREIGN KEY (match_id) REFERENCES badminton_matches(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

/*
  Table: badminton_match_summary
  Purpose: One-row summary per match (aggregate sets, winner, declared time)
*/
...
SELECT 'Badminton tables created successfully in sportssync.' AS status;
